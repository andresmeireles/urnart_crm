...
document.addEventListener(
  ...
  function (el) {
    if (el.target.classList.contains("auto-float-convert-blur")) {
      ...
      el.target
        .closest("div")
        .querySelector("[converted-value]").value = newValue;
    ...
    if (el.target.classList.contains("sum")) {
      ...
      let attName = div
        .querySelector("[sum-target]")
        .getAttribute("sum-target");
      ...
      let val = Intl.NumberFormat("pt-BR", {
        style: "currency",
        currency: "BRL",
      }).format(sum);
...
$(".calendar-selector").on("change", (element) => {
  let dateInput = element.target
    .closest("div")
    .querySelector(element.target.getAttribute("date-target"));
